...
**Rule.** `@RestResource` classes declare `global with sharing`. The session belongs to whichever user authenticated the inbound call — typically a dedicated integration user, never an interactive human session — and that user's sharing rules and field-level security govern what the endpoint can read and write. Document the assumed caller identity in the class header, and grant Apex Class Access to the endpoint through that integration user's own permission set, not through a profile.
